...
const { streamGemini } = require('../lib/gemini-stream');
const { checkRateLimit } = require('../lib/rateLimit');
const { requireUser } = require('../lib/verifyAuth');
const { MAX_ANSWER_LEN, MAX_HISTORY_MESSAGES, MAX_HISTORY_TEXT_LEN, MAX_GRADE_LABEL_LEN, MAX_SUBJECT_LABEL_LEN, MAX_TOPIC_LEN } = require('../lib/limits');
const { clamp } = require('../lib/util');
const { SMART_MODEL } = require('../lib/models');
const { sanitizeForAI, safeAIMessage } = require('../lib/guardrails');
const { getLearnerRubric } = require('../lib/learnerRubric');
const { retrieveCurriculumContext, formatRAGContext } = require('../lib/rag');
const { persistStudentTurn, persistAiTurn, getOwnedSession } = require('../lib/sessionPersistence');
...
async function handler(req, res) {
  ...
  const ownedSession = await getOwnedSession(require('../lib/supabaseAdmin').admin(), sessionId, userId);
  if (!ownedSession) return res.status(403).json({ error: 'forbidden', message: 'الجلسة دي مش بتاعتك.' });
  const dbForRetry = require('../lib/supabaseAdmin').admin();
  const { data: priorAi } = await dbForRetry.from('messages').select('content')
  ...
  const studentPersist = await persistStudentTurn({ sessionId, userId, turnId, content: answerSafe.text });
