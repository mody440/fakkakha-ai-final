...
async function handler(req, res) {
  ...
  const languageSample = [answerSafe, ...historySanitized.filter(m => m.role === 'student').map(m => m.safe.text), topic, subjectLabel].filter(Boolean).join(' ');
  ...
  res.setHeader('Connection', 'keep-alive');
  res.flushHeaders?.();
  // Keep Vercel/mobile proxies from treating a slow Gemini generation as a
  // dead connection. SSE comments are ignored by the client parser but keep
  // the transport alive while the model is working.
  ...
  try {
    full = await streamGemini(
      ...
      [{ text: prompt }], { model: SMART_MODEL, timeoutMs: 50000 },
