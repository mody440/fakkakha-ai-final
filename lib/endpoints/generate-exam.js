...
async function handler(req, res) {
  ...
  // Production profiles use id = auth.uid(); older schemas may also expose
  // a separate user_id. Accept either representation, but never trust the
  // client-provided profileId alone.
  const { data: profile } = await db.from('profiles').select('*').eq('id', profileId).maybeSingle();
  const profileOwnerId = profile?.user_id || profile?.id;
  if (!profile || profileOwnerId !== userId) {
