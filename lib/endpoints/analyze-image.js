...
async function handler(req, res) {
  ...
  // Keep a safe margin below Vercel's request-body limit after JSON overhead.
  ...
  const result = await callAndValidate(
    () => callGemini(SYSTEM_PROMPT, parts, { model: SMART_MODEL, timeoutMs: 40000 }),
    isValidShape,
    1
  );
  ...
  if (!result.ok) {
    ...
    return res.status(200).json({
      readable: false,
      extractedText: result.error === 'TIMEOUT'
        ? 'تحليل الصورة أخذ وقتًا أطول من المتوقع. جرّب صورة أوضح وأصغر، أو اكتب السؤال يدويًا.'
        : 'مقدرناش نحلل الصورة دلوقتي. جرّب صورة أوضح وأصغر، أو اكتب السؤال يدويًا.',
      fallback: true
    });
  }
